...
have not yet added `ghcib-plugin`.
...
coupling to the project's own build: cabal compiles `ghcib-plugin` against the right
...
The daemon communicates the plugin socket path by setting `GHCIB_PLUGIN_SOCKET` in
...
- No `-fplugin-opt` in user cabal files — only `-fplugin=Ghcib.Plugin` is needed.
...
## Component: ghcib-plugin

A new library, separate from the main `ghcib` library. Users add it once per watched
cabal component:

```cabal
library
  build-depends: ..., ghcib-plugin
  ghc-options:   -fplugin=Ghcib.Plugin
```

The plugin registers two hooks:

- **`driverPlugin`** — fires once at GHCi session startup. Reads
  `GHCIB_PLUGIN_SOCKET`; if present, wraps GHC's Logger to forward every diagnostic
...
When `GHCIB_PLUGIN_SOCKET` is absent the plugin returns immediately without
modifying the session.

---

## Component: ghcib Daemon Changes

| Component | Change |
|---|---|
| `ghcid` dependency | **Retained** as fallback |
| `Ghcib.Effects.GhciSession` (interpreter) | **Extended** — detects plugin connection; falls back to ghcid |
| `Ghcib.BuildState.Message` | **Extended** — add `errorCode`, `hints`, `relatedSpans` (empty in fallback mode) |
| `Ghcib.GhciSession` (component) | **Minor** — unchanged overall structure |
...
No changes to `.ghcib.toml`. Without `ghcib-plugin`, the daemon operates in fallback
mode automatically.
...
2. **GHCi without cabal** — plain `ghci` invocations require the user to pass
   `-fplugin=Ghcib.Plugin` themselves. Document this; do not attempt to auto-detect.
