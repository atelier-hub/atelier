...
```cabal
library ghcib-plugin
  exposed-modules: Ghcib.Plugin
  build-depends:
      base
    , ghc      >= 9.8
    , aeson
...
```haskell
module Ghcib.Plugin (plugin) where

plugin :: Plugin
plugin = defaultPlugin
  { driverPlugin          = ghcibDriverPlugin
  , typeCheckResultAction = \_ _ env -> pure env  -- no-op in prototype
  , pluginRecompile       = purePlugin
  }

ghcibDriverPlugin :: [CommandLineOption] -> HscEnv -> IO HscEnv
ghcibDriverPlugin _opts env = do
  mPath <- lookupEnv "GHCIB_PLUGIN_SOCKET"
  case mPath of
...
```
$XDG_RUNTIME_DIR/ghcib/<hash>.sock          -- client-facing (unchanged)
$XDG_RUNTIME_DIR/ghcib/<hash>-plugin.sock   -- plugin-facing (new)
```
...
3. Set GHCIB_PLUGIN_SOCKET=<path> in subprocess environment
