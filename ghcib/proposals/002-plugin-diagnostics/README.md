...
GHC diagnostics carry more information than ghcib currently surfaces. Error codes,
...
This limits what editors and tooling built on ghcib can do with build output.

This work package extends ghcib to expose that richer diagnostic data by introducing
`ghcib-plugin`, an optional GHC compiler plugin users add to their project. Projects
that do not install the plugin continue to work as before with no user-visible change.

**Deliverables:**
- `ghcib-plugin` library: GHC compiler plugin with Logger wrapping and socket
  forwarding
...
- User documentation: how to add `ghcib-plugin` to a cabal file
...
- A project with `ghcib-plugin` installed emits `Message` values with non-empty
  `errorCode` fields for GHC errors that carry codes.
- A project without `ghcib-plugin` continues to work as before, with the daemon
...
### Milestone 1 — ghcib-plugin Library

**Deliverables:**
- `Ghcib.Plugin` module with `plugin :: Plugin` entry point
- `driverPlugin` hook: reads `GHCIB_PLUGIN_SOCKET` from env; no-op if absent
...
**Acceptance criteria:**
Running `cabal repl` on a project with `-fplugin=Ghcib.Plugin` and
`GHCIB_PLUGIN_SOCKET` set produces `DiagnosticEvent` JSON on the socket for each GHC
...
- Startup: daemon opens socket, sets `GHCIB_PLUGIN_SOCKET`, spawns GHCi, waits up
...
**Acceptance criteria:**
- With plugin installed: `ghcib status` returns messages with `errorCode` populated
  for errors that carry GHC error codes.
- Without plugin installed: `ghcib status` returns messages as before; daemon log
  shows `"ghcib-plugin not detected; falling back to basic diagnostics"`.
...
- User-facing documentation: how to add `ghcib-plugin`, what the new fields contain,
