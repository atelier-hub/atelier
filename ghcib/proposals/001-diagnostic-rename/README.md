...
accurately describes what the type holds. Renaming it aligns ghcib's vocabulary with
...
- **Align terminology with the ecosystem** — `Diagnostic` is the term used by GHC,
  LSP, and most editor integrations; using it in ghcib reduces cognitive friction.
...
**Deliverables:**
- `Ghcib.BuildState.Message` → `Ghcib.BuildState.Diagnostic`
...
The `messages` → `diagnostics` field rename is a breaking change. Since ghcib has no
