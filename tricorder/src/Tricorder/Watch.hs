...
import Brick.Widgets.Core (Padding (..), emptyWidget, hBox, padLeft, txt, withDefAttr)
...
viewTargets targets =
    hBoxSpaced
        ...
        , if null targets then
            ...
            txt (T.intercalate " " targets)
        ]


viewMetrics :: Maybe Int -> Widget n
viewMetrics Nothing =
    hBoxSpaced 1 [emphasis $ txt "Metrics:", warn $ txt "disabled"]
...
hBoxSpaced space = hBox . intersperse spacing
  where
    spacing = hBox $ replicate space $ txt " "


vBoxSpaced :: Int -> [Widget n] -> Widget n
vBoxSpaced space = vBox . intersperse spacing
  where
    spacing = vBox $ replicate space $ txt " "
